...
import { aircallDaily } from "@/data/aircall";
...
const MONTHLY_TARGET = DAILY_TARGET * WORKING_DAYS_MONTH;

export function TargetTracker() {
  const daily = aircallDaily;
  ...
  const sollBisher = DAILY_TARGET * daysElapsed;
  ...
  const remainingDials = MONTHLY_TARGET - totalDials;
  ...
  const maxDials = Math.max(...daily.map((d) => d.dials), DAILY_TARGET);
        ...
        <div className="glass-card p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-[11px] text-[#57534e] mt-0.5">{DAILY_TARGET}/Tag × {daysElapsed} Tage</div>
            </div>
      ...
      <div className="glass-card p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-[13px] text-[#57534e]">Tägliche Outbound-Calls im Vergleich zum SOLL</p>
          </div>
        </div>
          ...
          <BarChart data={chartData} barCategoryGap="20%">
            ...
            <Bar dataKey="dials" radius={[4, 4, 0, 0]} animationDuration={600}>
              {chartData.map((entry, i) => (
                <Cell
                  key={i}
                  fill={entry.dials >= DAILY_TARGET ? "#818cf8" : "rgba(129,140,248,0.5)"}
                />
              ))}
            </Bar>
          </BarChart>
        ...
        <div className="mt-5 pt-4 border-t border-[rgba(255,255,255,0.04)]">
          <div className="flex items-center justify-between text-[11px] mb-2">
            <span className="text-[#78716c]">Monatsziel: {MONTHLY_TARGET} Dials</span>
            <span className="tabular-nums font-medium text-[#a8a29e]">{totalDials} / {MONTHLY_TARGET} ({Math.round((totalDials / MONTHLY_TARGET) * 100)}%)</span>
          </div>
          <div className="h-2 rounded-full bg-[rgba(255,255,255,0.04)] overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                totalDials >= MONTHLY_TARGET
                  ? "bg-gradient-to-r from-[#5eead4] to-[#2dd4bf]"
                  : "bg-gradient-to-r from-[#818cf8] to-[#6366f1]"
              }`}
              style={{ width: `${Math.min(100, Math.round((totalDials / MONTHLY_TARGET) * 100))}%` }}
            />
          </div>
        </div>
      </div>
